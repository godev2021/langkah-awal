...
public class ThreeSixtyController {
    ...
    public ResponseEntity<ApiResponse<ThreeSixtyBean>> getDetail(@PathVariable Long employeeId) {
        ThreeSixtyBean threeSixtyByEmployeeId = threeSixtyReviewService.getThreeSixtyByEmployeeId(employeeId);
        return ResponseEntity.ok(new ApiResponse<>(true, threeSixtyByEmployeeId, null));
    }
}
